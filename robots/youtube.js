...
async function robot(){
	...
	async function authenticatedWithOAuth(){
		const webServer = await startWebSever();
		const OAuthClient = await createOAuthClient();
		requestUserConsent(OAuthClient);
		const authorizationToken = await waitForGoogleCallback(webServer);
		await requestGoogleForAccessTokens(OAuthClient, authorizationToken);
		await setGlobalGoogleAuthentication(OAuthClient);
		await stopWebSever(webServer);

		async function startWebSever(){
		...
		async function waitForGoogleCallback(webServer){
			return new Promise((resolve, reject) =>{
				...
				webServer.app.get('/oauth2callback', (req, res) =>{
					...
					res.send('<h1>Thank you!</h1></p>Now close thsi tab.</p>');
					resolve(authCode);
				});
			});
		}
		...
		async function stopWebSever(webServer){
			return new Promise((resolve, reject) =>{
